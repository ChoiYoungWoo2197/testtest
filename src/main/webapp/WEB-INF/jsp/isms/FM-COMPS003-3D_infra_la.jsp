	...
	<div class="talbeArea">
		<table summary="통제항목설정">
			...
			<thead>
				...
				<tr>
					<th scope="col">착안사항 설명</th>
					<th colspan="2" scope="col">증빙자료예시</th>
				</tr>
			</thead>
			<tbody id="tbody">
			 <c:forEach var="result" items="${resultList}" varStatus="status">
				<tr>
					<td rowspan="6">${result.ucm3lvCod}</td>
					<td style="text-align: left">${result.ucm3lvNam}</td>
					<td>
						<div class="uplidFileList">
							<ul class="listArea" style="width: 190px; height: auto; min-height: 30px; max-height:96px;">
				 ...
				 <c:forEach var="expData" items="${listExpData}" varStatus="status2">
					 <c:if test="${expData.ctrKey==result.ucmCtrKey&&expData.lvlCod==result.ucm3lvCod}">
						 <tr>
						 	<td class="expData" style="text-align: left"><span class="seq">${expData.seq}</span>${expData.title}</td>
						 	<td colspan="3" class="last" style="text-align: left; white-space: pre-wrap;">${expData.note}</td>
						 </tr>
					 </c:if>
				 </c:forEach>
			</c:forEach>
