...
<head>
	...
	<script>
		...
		function makePDF(){
			//글자 겹침 현상 발생 방지
			// $("#wrap td").each(function(i, v){
			// 	$("#wrap td").eq(i).text($("#wrap td").eq(i).text().replace(/\(/g, " ("));
			// });
								...
								<table class="task-content-table mb-5">
									...
									<tr>
										<td class="text-left" style="min-height: 3rem">
											<div>${taskNode.data.taskContent}</div>
