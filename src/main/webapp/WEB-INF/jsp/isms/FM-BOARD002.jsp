...
<script type="text/javascript">
	...
	function fn_egov_pageview(pageNo) {
		...
		document.listForm.action = "FM-BOARD002.do";
		document.listForm.submit();
	}

	function fn_egov_regist_Restde() {
		document.listForm.action = "FM-BOARD002_RW.do";
		document.listForm.submit();
	}

	function fn_egov_select(id) {
		document.listForm.selectedId.value = id;
		document.listForm.action = "FM-BOARD002_V.do";
		document.listForm.submit();
	}

	function gubunChange(){
		var selectCondition = $("#searchCondition").val();
		var html='';
		if(selectCondition != '3'){
			html = "<input id='searchKeyword' name='searchKeyword' value='${searchVO.searchKeyword}' class='inputText' type='text'	title='검색어 입력' placeholder='검색어를 입력하세요.'/>";
		}else{
			html = "<sb:select name='searchKeyword' type='A' code='CATEGORY' value='${searchVO.searchKeyword}'/>";
		}
		$("#searchGubun").html(html);
	}
...
<body>
		...
		<form name="listForm" action="FM-BOARD002.do" method="post">
		...
		<div class="search">
			<div class="defalt">
				<fieldset class="searchForm">
					<legend>기본검색</legend>
					<ul class="defalt">
						<li><select id="searchCondition" class="selectBox" title="검색항목 선택"  name="searchCondition" onchange="gubunChange();">
                				...
                				<option value="3" <c:if test="${searchVO.searchCondition == '3'}">selected="selected"</c:if>>분류</option>
						</select></li>
						<li id="searchGubun"><input id="searchKeyword" name="searchKeyword" value="${searchVO.searchKeyword}" class="inputText" type="text"	title="검색어 입력" placeholder="검색어를 입력하세요."/></li>
					</ul>
				...
				<table summary="보안정책자료실">
					...
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr>
							...
							<td><c:out value="${result.ubmCatCod}"/></td>
