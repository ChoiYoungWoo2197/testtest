...
//				if(data.cnt != 0 && data.depth == 'CNTR3' ){
//					nodeTitle = nodeTitle + '<span style="display:inline-block;"' + '>(양식)</span>';
//				}
			}
		}
		res += nodeTitle;
...
//          dtnode._fixSelectionState();
		...
		if( dtnode.bSelected && opts.selectMode==3 ) {
			...
			while( p ) {
				if( !p.hasSubSel ){
					// 2013-12-02 선택시 상위 노드에 select 색칠해지는 부분 주석
					//p._setSubSel(true);
				}
				p = p.parent;
			}
